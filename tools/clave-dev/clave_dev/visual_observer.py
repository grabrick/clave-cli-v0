...
def sweep_dead_windows(osa=None):
    ...
    не закрывается НИЧЕМ. Проверено на живом зомби, все три дороги:
      * `close (every window whose id is …) saving no` — рапортует успех, окно на месте;
      * `close (every window whose name contains …) saving no` — то же самое;
      * System Events (клик по AXCloseButton) — окно ВООБЩЕ НЕ ВИДИТ: в списке Terminal оно есть,
        а в UI-иерархии его нет.
    Дальше идти некуда, и это надо было измерить, а не предполагать.
