...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--severity-threshold", default="none",
                   choices=["none", "low", "medium", "high"],
                   help="минимальный severity находки, который блокирует (§6). ДЕФОЛТ 'none': "
                        "блокирует только провал required-чеклиста, мнения модели идут агенту "
                        "справочно. Поднимать порог можно для разовой строгой диагностики, но НЕ "
                        "для автономной петли: issue — свободный текст, его нельзя ни вычесть по "
                        "базовой линии, ни набрать консенсусом, а судья охотно выдаёт medium-мнения "
                        "и на здоровой сборке — петля не сойдётся")
