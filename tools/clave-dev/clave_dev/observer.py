...
def observer_preflight() -> str:
    """Причина, по которой наблюдение не сможет работать, или пустая строка.

    Импорт `pyte` ниже ленивый — и это правильно: чистая логика петли тестируется без него. Но
    цена ошибки была чудовищной. Без `pyte` супервайзер падал не на старте, а на стадии
    наблюдения — то есть ЧЕРЕЗ ДЕСЯТЬ МИНУТ, уже после того как агент написал код и прошёл все
    проверки. Работа не терялась (она в worktree), но прогон выбрасывался целиком, а причину
    приходилось выкапывать из хвоста лога.

    Ровил меня на этом собственный `ps`: у venv-питона он печатает РЕЗОЛВНУТЫЙ бинарь, тот же
    самый, что у системного. Я посмотрел на вывод, решил, что прогоны идут на системном питоне,
    и запустил их им же — а `pyte` живёт в `.venv`.

    Проверка стоит миллисекунды и обязана быть на старте: гейт, который падает ПОСЛЕ всей работы,
    это гейт, выброшенный в мусор.
    """
    try:
        import pyte  # noqa: F401
    except ImportError:
        return (
            "pyte не установлен — наблюдать за экраном нечем. "
            "Супервайзер надо запускать венвовским питоном: "
            "cd tools/clave-dev && .venv/bin/python3 -m clave_dev ... "
            "(поставить: .venv/bin/pip install pyte)"
        )
    return ""
