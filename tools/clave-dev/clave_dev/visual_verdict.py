...
def verdict_passes(v: VisionVerdict, blocking=()) -> bool:
    ...
    `blocking` пуст по умолчанию, и это принципиально. Пункты чеклиста перечислимы и стабильны:
    их можно вычесть по базовой линии и потребовать совпадения в нескольких выборках. С issue так
    не выйдет — это свободный текст, и он у судьи каждый раз новый: ни сопоставить с базой, ни
    набрать по нему консенсус нельзя. Значит гейт по issue остаётся АБСОЛЮТНЫМ — ровно тем, из-за
    которого петля не сходилась никогда: судья охотно выдаёт мнения о продукте («логотип
    фрагментарный», «диакритик не отрисован» — оба приходили с severity=medium на здоровой
    сборке), и одного такого хватает, чтобы гонять агента чинить несломанное до конца раундов.

    Поэтому мнения — справочные: они уходят агенту в контекст, но не блокируют. Порог можно
    поднять вручную для разовой строгой диагностики, но не для автономной петли.
    """
