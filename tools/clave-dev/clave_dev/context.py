...
def build_flake_context(flaky) -> str:
    """Блок «Твои тесты падают вразброс» — обратная связь агенту.

    Сказать «почини флейк» мало: агент поднимет таймаут, тест снова позеленеет, и гонка вернётся
    в первый же нагруженный день. Требование должно быть про ПРИЧИНУ: убрать гонку, а не отсрочить
    её. И объяснить цену: флейк не шумит, он ВРЁТ — набор краснеет сам по себе, а мутационный гейт
    читает это как «мутант пойман» и объявляет покрытым то, что не покрыто ничем.
    """
    from .flake import describe

    lines = [
        "## Твои тесты падают вразброс",
        "",
        "Под параллельной нагрузкой набор упал, хотя одиночный прогон зелёный. Это не шум:",
        "мутационный гейт судит мутанта по цвету набора. Красный набор он засчитывает как",
        "«мутант пойман» — и объявляет покрытым код, который не покрыт ничем. Пока набор",
        "неустойчив, ни одной цифре в этом прогоне верить нельзя.",
        "",
    ]
    lines += [f"- {line}" for line in describe(list(flaky))]
    lines += [
        "",
        "Убери ГОНКУ, а не симптом. Поднять таймаут — не починка, а отсрочка. Обычные причины:",
        "общий на всю машину путь в /tmp (делай его уникальным на процесс: `std::process::id()`),",
        "наблюдение за состоянием в гонке с таймаутом, который тест сам же и проверяет,",
        "и правка переменных окружения, пока соседний тест их читает.",
    ]
    return "\n".join(lines)
