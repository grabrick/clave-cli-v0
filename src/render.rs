...
fn build_dynamic(app: &App, width: u16, full_h: u16) -> (Vec<Line<'static>>, u16, u16) {
    ...
    if let Some(reveal) = &app.reveal {
        ...
        // В тандеме живой `⏺`-стрим НЕ показываем: тот же текст через миг появится готовым
        // блоком `🅐`/`🅒` (шаги фиксируются в ленту по завершении), а стрим лишь двоил бы его
        // и раздувал живой блок. Прогресс шага видно по лоадеру и активности `⎿`.
        if !visible.is_empty() && !app.is_tandem_run {
...
mod tests {
    ...
    /// В прогоне тандема живой `⏺`-стрим НЕ показываем: тот же текст появится готовым блоком
    /// `🅐`/`🅒`, а стрим лишь двоил бы его и раздувал живой блок (был краш с тремя лоадерами).
    #[test]
    fn tandem_run_hides_the_live_stream_block() {
        let mut app = footer_app();
        app.running = true;
        app.is_tandem_run = true;
        app.live_answer = "стрим шага тандема".to_string();

        let (lines, _, _) = build_dynamic(&app, 80, 24);
        let joined: String = lines.iter().map(text_of).collect::<Vec<_>>().join("\n");
        assert!(
            !joined.contains("стрим шага тандема"),
            "текст стрима не показан: {joined}"
        );
        assert!(!joined.contains('⏺'), "нет ⏺-блока: {joined}");
    }
