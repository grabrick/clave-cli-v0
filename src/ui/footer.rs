...
mod tests {
    ...
    ///
    /// Мутанта «всегда true» это убивает в чистом окружении, «всегда false» — под супервайзером.
    /// Обоих сразу одним прогоном не убить: для этого пришлось бы менять переменную, то есть
    /// вернуть ту самую гонку. Граница объявлена вслух у `static_render`.
    #[test]
